...
fileQueue.process(async (job, done) => {
  ...
  const userId = job.data.userId || null; // Corrected from UserId to userId

  if (!fileId) {
    throw new Error('Missing fileId'); // Corrected from 'Missing fieldId' to 'Missing fileId'
  }
  if (!userId) {
